  ...
  return (
    <>
      <Container className="border-divide border-x">
        <h2 className="pt-11 pb-10 text-center font-mono text-sm tracking-tight text-neutral-500 uppercase dark:text-neutral-400">
          Trusted by Designers, Engineers, and Analysts
        </h2>
      </Container>
      <DivideX />
      <Container className="border-divide relative border-x">
        ...
        <div className="border-divide border-t w-full">
          {testimonials.slice(0, 8).map((testimonial, index) => {
            return (
              <button
                key={testimonial.src + index}
                className={cn(
                  "border border-divide group relative overflow-hidden w-full"
                )}
                onClick={() => {
              ...
              >
                ...
                <AnimatePresence initial={false} mode="wait">
                  <motion.div
                    key={testimonial.src + index}
                    className="group flex w-full h-full items-center justify-center p-4 py-10 opacity-70 grayscale transition-all duration-500 hover:opacity-100"
                    initial={{
                      y: 80,
                      opacity: 0,
                    }}
